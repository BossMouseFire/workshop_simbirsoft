...
export const getCoordinates = async (address: string) => {
  ...
  const response = await axios.get<IRequestCoordinates>(
...
export const getOrderStatus = async () => {
  return await instanceApiFactory.get<IRequestOrderStatus>(
    "/api/db/orderStatus/"
  );
};
...
export const postOrder = async (body: IBodyOrder) => {
  ...
  return await instanceApiFactory.post<IResponseOrder>("/api/db/order", json, {
    headers: {
...
export const cancelOrder = async (id: string) => {
  ...
  return await instanceApiFactory.put<IResponseOrder>(
    `/api/db/order/${id}`,
    formData
  );
};
