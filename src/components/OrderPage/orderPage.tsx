...
const OrderPage: React.FC = () => {
  ...
  const [responseOrder, setResponseOrder] = useState<IResponseOrder>(
    {} as IResponseOrder
  );
  ...
  useEffect(() => {
    ...
    if (id && id !== "order") {
      getOrderById(id)
        .then((response) => {
          if (
            response.data.data.orderStatusId.id === "5e26a1f5099b810b946c5d8c"
          ) {
            setCurrentBlock(5);
          } else {
            setCurrentBlock(4);
        ...
        <>
          <div className={styles.blockActions}>
            ...
            <div className={styles.actions}>
              <span
                className={cx({ activatedAction: currentBlock === 0 })}
